...
public class AnomalyDetector {
    ...
    public void tune(TimeSeries.DataSequence expectedValues) throws Exception {
        ...
        for (AnomalyDetectionModel model : models) {
            if (!isTuned.get(i)) {
//                for (int j = 0; j < metric.data.getValues().length; j++) {
//                    System.out.println("原始值和预测值：" + String.valueOf(metric.data.getValues()[j]) + "," + String.valueOf(expectedValues.getValues()[j]));
//                }
